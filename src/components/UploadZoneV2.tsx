...
interface UploadZoneV2Props {
  onFileSelect: (file: File) => void;
  isProcessing: boolean;
  previewUrl: string | null;
}

export function UploadZoneV2({ onFileSelect, isProcessing, previewUrl }: UploadZoneV2Props) {
  ...
  return (
    <div className="w-full">
      <div
        ...
        onDrop={handleDrop}
        className={`upload-panel-frosted cursor-pointer text-center ${isDragActive ? 'drag-active' : ''}`}
      >
        <input
          ref={inputRef}
          type="file"
        ...
        <div className="space-y-6 py-8">
          ...
          <div className={`
            ...
            ${isDragActive
              ...
              : 'border-[var(--color-border)] bg-resin-mid/50 hover:border-brick/50'
            }
          `}>
            <svg
              className={`h-8 w-8 transition-colors duration-300 ${isDragActive ? 'text-brick' : 'text-secondary'}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.5}
          ...
          <div>
            <p className="text-lg font-medium text-primary mb-2">
              {isDragActive ? 'Drop it here' : 'Upload Collection'}
            </p>
            <p className="text-sm text-secondary">
              Drag & drop your G-Shock photo
            </p>
          </div>
